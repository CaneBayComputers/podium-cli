...
IDEA_STIRLING='Deploy Stirling PDF on this Zeltro server. The project name is stirling-pdf — use this exact name, no changes.
...
3. cd ~/zeltro-projects/stirling-pdf && zeltro setup stirling-pdf --no-startup && zeltro up stirling-pdf
4. curl -sI http://stirling-pdf/ — expect 200.'"$SUMMARY_SUFFIX"

IDEA_ITTOOLS='Deploy IT Tools on this Zeltro server. The project name is it-tools — use this exact name, no changes.
...
3. cd ~/zeltro-projects/it-tools && zeltro setup it-tools --no-startup && zeltro up it-tools
4. curl -sI http://it-tools/ — expect 200.'"$SUMMARY_SUFFIX"

IDEA_CHANGEDETECTION='Deploy Changedetection.io on this Zeltro server. The project name is changedetection — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/changedetection
...
3. cd ~/zeltro-projects/changedetection && zeltro setup changedetection --no-startup && zeltro up changedetection
4. curl -sI http://changedetection/ — expect 200.'"$SUMMARY_SUFFIX"

IDEA_FLAME='Deploy Flame on this Zeltro server. The project name is flame — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/flame
...
3. cd ~/zeltro-projects/flame && zeltro setup flame --no-startup && zeltro up flame
4. curl -sI http://flame/ — expect 200. Password: admin.'"$SUMMARY_SUFFIX"

IDEA_HEIMDALL='Deploy Heimdall on this Zeltro server. The project name is heimdall — use this exact name, no changes.
...
3. cd ~/zeltro-projects/heimdall && zeltro setup heimdall --no-startup && zeltro up heimdall
4. curl -sI http://heimdall/ — expect 200.'"$SUMMARY_SUFFIX"

IDEA_UMAMI='Deploy Umami on this Zeltro server. The project name is umami — use this exact name, no changes.

Umami is a privacy-focused analytics platform (image: ghcr.io/umami-software/umami:postgresql-latest). It listens on port 3000. Use an nginx reverse proxy. IMPORTANT: Umami supports PostgreSQL ONLY. Use the shared zeltro-postgres (host=zeltro-postgres, port=5432, user=root, password=password).

Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE umami;"
2. mkdir -p ~/zeltro-projects/umami
3. Write docker-compose.yaml: umami-app service (env: DATABASE_URL=postgresql://root:password@zeltro-postgres:5432/umami, APP_SECRET=any-random-32-char-string) + nginx (container_name=umami, static VPC IP). Write nginx.conf proxying to umami-app:3000.
4. cd ~/zeltro-projects/umami && zeltro setup umami --no-startup && zeltro up umami
5. curl -sI http://umami/ — expect 200. Default: admin/umami.'"$SUMMARY_SUFFIX"

IDEA_DASHY='Deploy Dashy on this Zeltro server. The project name is dashy — use this exact name, no changes.
...
3. cd ~/zeltro-projects/dashy && zeltro setup dashy --no-startup && zeltro up dashy
4. curl -sI http://dashy/ — expect 200.'"$SUMMARY_SUFFIX"

IDEA_LIMESURVEY='Deploy LimeSurvey on this Zeltro server. The project name is limesurvey — use this exact name, no changes.
...
Steps:
1. Create DB user: docker exec zeltro-mariadb mysql -u root -e "CREATE DATABASE IF NOT EXISTS limesurvey; CREATE USER IF NOT EXISTS '"'"'limesurvey'"'"'@'"'"'%'"'"' IDENTIFIED BY '"'"'limesurvey'"'"'; GRANT ALL PRIVILEGES ON limesurvey.* TO '"'"'limesurvey'"'"'@'"'"'%'"'"'; FLUSH PRIVILEGES;"
2. mkdir -p ~/zeltro-projects/limesurvey
3. Write docker-compose.yaml: limesurvey-app service (env: DB_TYPE=mysql, DB_HOST=zeltro-mariadb, DB_PORT=3306, DB_NAME=limesurvey, DB_USERNAME=limesurvey, DB_PASSWORD=limesurvey, ADMIN_USER=admin, ADMIN_NAME=Administrator, ADMIN_EMAIL=admin@example.com, ADMIN_PASSWORD=admin123, PUBLIC_URL=http://limesurvey; persist /var/www/html/upload) + nginx (container_name=limesurvey, static VPC IP). Write nginx.conf proxying to limesurvey-app:8080.
4. cd ~/zeltro-projects/limesurvey && zeltro setup limesurvey --no-startup && zeltro up limesurvey
