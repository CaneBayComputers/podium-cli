...
mkdir -p /tmp/zeltro-tests/custom-sessions
LOG=/tmp/zeltro-tests/custom-master.log
...
run_project() {
    local name="$1"
    local idea="$2"
    local logfile="/tmp/zeltro-tests/custom-sessions/${name}.log"
...
## Commands Run
[bulleted list of key zeltro/docker commands you ran]
## Issues & Fixes
...
IDEA1='Build a Python book club manager using `zeltro new python book-club --no-github`. Track books: title, author, genre (fiction/non-fiction/sci-fi/mystery/biography/other), page_count, date_read_by_club, group_rating (1-5), and a short group_verdict text. Add discussion_notes per book: date and note_text. Reading stats page returns JSON with: books per month (last 6 months), most common genre, average group rating, total pages read, and top-rated book. Use MySQL/MariaDB. Seed with 8 books across genres and their discussion notes.'"$SUMMARY_SUFFIX"

IDEA2='Build a Django household budget tracker using `zeltro new django budget-tracker --no-github`. Track income and expenses: amount, type (income/expense), category (housing/food/transport/entertainment/utilities/health/other), date, description. Monthly budget limits are set per category. A monthly summary endpoint returns: total income, total expenses, spending by category as a percentage of its budget limit, and whether the month is under or over budget overall. Use MySQL/MariaDB. Seed with monthly budget limits and 3 months of realistic transactions.'"$SUMMARY_SUFFIX"

IDEA3='Build a Laravel restaurant reviewer using `zeltro new laravel restaurant-reviews --no-github`. Restaurants: name, cuisine (Italian/Japanese/Mexican/American/Thai/Indian/other), neighborhood, price_range (1-3 representing $/$$/$$$), phone, website. Reviews: restaurant_id, reviewer_name, rating (1-5), visit_date, dish_ordered, review_text. Homepage lists restaurants sorted by average rating with review count. Detail page shows all reviews for a restaurant. Filter by cuisine or price range. Use MySQL/MariaDB. Seed with 8 restaurants and 3+ reviews each.'"$SUMMARY_SUFFIX"

IDEA4='Build a WordPress events board. Install WordPress using `zeltro new wordpress events-board --no-github`. After setup, use WP-CLI (`zeltro wp`) to: register a custom post type "event" with meta fields for event_date, location, category (music/sports/food/arts/community), organizer, and is_free (yes/no). Create a page template that lists upcoming events sorted by date with category filter. Use `zeltro wp post create` to seed 10 upcoming events across all categories with realistic titles, descriptions, and dates in the next 60 days.'"$SUMMARY_SUFFIX"
