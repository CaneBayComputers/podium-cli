...
IDEA_REDASH='Deploy Redash on this Zeltro server. The project name is redash — use this exact name, no changes.
...
Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE redash;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/redash
...
4. Write docker-compose.yaml with TWO app services:
   - redash-server: image redash/redash:latest, command: server, env: REDASH_DATABASE_URL=postgresql://root:password@zeltro-postgres/redash, REDASH_REDIS_URL=redis://zeltro-redis:6379/3, REDASH_SECRET_KEY=<generated>, REDASH_COOKIE_SECRET=<generated>, PYTHONUNBUFFERED=0
...
5. Run migrations first: docker run --rm --network zeltro-cli_vpc -e REDASH_DATABASE_URL=postgresql://root:password@zeltro-postgres/redash redash/redash:latest manage db upgrade
6. Create admin: docker run --rm --network zeltro-cli_vpc -e REDASH_DATABASE_URL=postgresql://root:password@zeltro-postgres/redash redash/redash:latest manage users create_root admin@example.com admin --password admin123
7. cd ~/zeltro-projects/redash && zeltro setup redash --no-startup && zeltro up redash
8. curl -sI http://redash/ — expect 200 or 302.'"$SUMMARY_SUFFIX"

IDEA_ARCHIVEBOX='Deploy ArchiveBox on this Zeltro server. The project name is archivebox — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/archivebox
...
3. cd ~/zeltro-projects/archivebox && zeltro setup archivebox --no-startup && zeltro up archivebox
4. Wait 30 seconds for initialization, then curl -sI http://archivebox/ — expect 200 or 302.'"$SUMMARY_SUFFIX"

IDEA_KAVITA='Deploy Kavita on this Zeltro server. The project name is kavita — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/kavita
...
3. cd ~/zeltro-projects/kavita && zeltro setup kavita --no-startup && zeltro up kavita
4. curl -sI http://kavita/ — expect 200 or 302. Visit http://kavita/ to create admin account.'"$SUMMARY_SUFFIX"

IDEA_AUDIOBOOKSHELF='Deploy Audiobookshelf on this Zeltro server. The project name is audiobookshelf — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/audiobookshelf
...
3. cd ~/zeltro-projects/audiobookshelf && zeltro setup audiobookshelf --no-startup && zeltro up audiobookshelf
4. curl -sI http://audiobookshelf/ — expect 200 or 302. Visit http://audiobookshelf/ to create admin account.'"$SUMMARY_SUFFIX"

IDEA_NGINXPROXYMANAGER='Deploy Nginx Proxy Manager on this Zeltro server. The project name is nginx-proxy-manager — use this exact name, no changes.

Nginx Proxy Manager is a reverse proxy management UI (image: jc21/nginx-proxy-manager:latest). The admin UI listens on port 81. Use an nginx reverse proxy (from Zeltro) to expose the admin UI at port 80. Uses SQLite internally — no external database needed.

Steps:
1. mkdir -p ~/zeltro-projects/nginx-proxy-manager
...
3. cd ~/zeltro-projects/nginx-proxy-manager && zeltro setup nginx-proxy-manager --no-startup && zeltro up nginx-proxy-manager
...
IDEA_BABYBUDDY='Deploy BabyBuddy on this Zeltro server. The project name is babybuddy — use this exact name, no changes.

BabyBuddy is a baby tracking app (image: lscr.io/linuxserver/babybuddy:latest). It listens on port 8000. Use an nginx reverse proxy. Use the shared PostgreSQL (host=zeltro-postgres, port=5432, user=root, password=password).

Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE babybuddy;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/babybuddy
3. Write docker-compose.yaml: babybuddy-app service (image: lscr.io/linuxserver/babybuddy:latest; env: PUID=1000, PGID=1000, TZ=UTC, DB_ENGINE=django.db.backends.postgresql, DB_HOST=zeltro-postgres, DB_PORT=5432, DB_NAME=babybuddy, DB_USER=root, DB_PASSWORD=password, CSRF_TRUSTED_ORIGINS=http://babybuddy; volume: babybuddy-config:/config) + nginx (container_name=babybuddy, static VPC IP). Write nginx.conf proxying to babybuddy-app:8000.
4. cd ~/zeltro-projects/babybuddy && zeltro setup babybuddy --no-startup && zeltro up babybuddy
5. curl -sI http://babybuddy/ — expect 200 or 302. Default login: admin / admin.'"$SUMMARY_SUFFIX"

IDEA_SUPERSET='Deploy Apache Superset on this Zeltro server. The project name is superset — use this exact name, no changes.

Apache Superset is a data exploration and visualization platform (image: apache/superset:latest). It listens on port 8088. Use an nginx reverse proxy. Use the shared PostgreSQL (host=zeltro-postgres, port=5432, user=root, password=password).

Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE superset;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/superset
3. Generate SECRET_KEY: $(openssl rand -hex 32)
4. Write docker-compose.yaml with TWO services:
   - superset-init: image apache/superset:latest, restart: "no", command: bash -c "superset db upgrade && superset fab create-admin --username admin --firstname Admin --lastname User --email admin@example.com --password admin123 && superset init", env: SUPERSET_SECRET_KEY=<generated>, SQLALCHEMY_DATABASE_URI=postgresql+psycopg2://root:password@zeltro-postgres/superset
...
5. cd ~/zeltro-projects/superset && zeltro setup superset --no-startup && zeltro up superset
6. Wait 60 seconds for init to complete, then curl -sI http://superset/ — expect 200 or 302. Login: admin / admin123.'"$SUMMARY_SUFFIX"

IDEA_ROUNDCUBE='Deploy Roundcube on this Zeltro server. The project name is roundcube — use this exact name, no changes.

Roundcube is an open-source webmail client (image: roundcube/roundcubemail:latest). It serves on port 80 directly — no nginx proxy needed. Use the shared MariaDB (host=zeltro-mariadb, user=root, password=empty). Pre-configure to use Zeltro'"'"'s MailHog for local development.

Steps:
1. Create DB: docker exec zeltro-mariadb mariadb -u root -e "CREATE DATABASE IF NOT EXISTS roundcube CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
2. mkdir -p ~/zeltro-projects/roundcube
3. Write docker-compose.yaml: single service roundcube-app (image: roundcube/roundcubemail:latest; env: ROUNDCUBEMAIL_DB_TYPE=mysql, ROUNDCUBEMAIL_DB_HOST=zeltro-mariadb, ROUNDCUBEMAIL_DB_PORT=3306, ROUNDCUBEMAIL_DB_NAME=roundcube, ROUNDCUBEMAIL_DB_USER=root, ROUNDCUBEMAIL_DB_PASSWORD=, ROUNDCUBEMAIL_DEFAULT_HOST=zeltro-mailhog, ROUNDCUBEMAIL_DEFAULT_PORT=1025, ROUNDCUBEMAIL_SMTP_SERVER=zeltro-mailhog, ROUNDCUBEMAIL_SMTP_PORT=1025, ROUNDCUBEMAIL_SKIN=elastic; container_name=roundcube, static VPC IP).
4. cd ~/zeltro-projects/roundcube && zeltro setup roundcube --no-startup && zeltro up roundcube
