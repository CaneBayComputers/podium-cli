...
IDEA_STRAPI='Deploy Strapi on this Zeltro server. The project name is strapi — use this exact name, no changes.

Strapi is a headless CMS (image: naskio/strapi:latest). It listens on port 1337. Use an nginx reverse proxy. Use the shared PostgreSQL (host=zeltro-postgres, port=5432, user=root, password=password).

Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE strapi;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/strapi
...
4. Write docker-compose.yaml: strapi-app service (image: naskio/strapi:latest; env: DATABASE_CLIENT=postgres, DATABASE_HOST=zeltro-postgres, DATABASE_PORT=5432, DATABASE_NAME=strapi, DATABASE_USERNAME=root, DATABASE_PASSWORD=password, APP_KEYS=<generated>, API_TOKEN_SALT=<generated>, ADMIN_JWT_SECRET=<generated>, TRANSFER_TOKEN_SALT=<generated>, JWT_SECRET=<generated>, NODE_ENV=production; volume: strapi-uploads:/opt/app/public/uploads) + nginx (container_name=strapi, static VPC IP). Write nginx.conf proxying to strapi-app:1337.
5. cd ~/zeltro-projects/strapi && zeltro setup strapi --no-startup && zeltro up strapi
...
IDEA_DIRECTUS='Deploy Directus on this Zeltro server. The project name is directus — use this exact name, no changes.

Directus is an open data platform / headless CMS (image: directus/directus:latest). It listens on port 8055. Use an nginx reverse proxy. Use the shared PostgreSQL (host=zeltro-postgres, port=5432, user=root, password=password).

Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE directus;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/directus
3. Generate SECRET: $(openssl rand -hex 32)
4. Write docker-compose.yaml: directus-app service (image: directus/directus:latest; env: SECRET=<generated>, DB_CLIENT=pg, DB_HOST=zeltro-postgres, DB_PORT=5432, DB_DATABASE=directus, DB_USER=root, DB_PASSWORD=password, ADMIN_EMAIL=admin@example.com, ADMIN_PASSWORD=admin123, PUBLIC_URL=http://directus; volume: directus-uploads:/directus/uploads) + nginx (container_name=directus, static VPC IP). Write nginx.conf proxying to directus-app:8055.
5. cd ~/zeltro-projects/directus && zeltro setup directus --no-startup && zeltro up directus
6. curl -sI http://directus/ — expect 200 or 302.'"$SUMMARY_SUFFIX"

IDEA_EXCALIDRAW='Deploy Excalidraw on this Zeltro server. The project name is excalidraw — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/excalidraw
...
3. cd ~/zeltro-projects/excalidraw && zeltro setup excalidraw --no-startup && zeltro up excalidraw
4. curl -sI http://excalidraw/ — expect 200.'"$SUMMARY_SUFFIX"

IDEA_MEILISEARCH='Deploy Meilisearch on this Zeltro server. The project name is meilisearch — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/meilisearch
...
4. cd ~/zeltro-projects/meilisearch && zeltro setup meilisearch --no-startup && zeltro up meilisearch
5. curl -sI http://meilisearch/ — expect 200. Access the search dashboard at http://meilisearch/.'"$SUMMARY_SUFFIX"

IDEA_METABASE='Deploy Metabase on this Zeltro server. The project name is metabase — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/metabase
...
3. cd ~/zeltro-projects/metabase && zeltro setup metabase --no-startup && zeltro up metabase
...
IDEA_HEALTHCHECKS='Deploy Healthchecks on this Zeltro server. The project name is healthchecks — use this exact name, no changes.

Healthchecks is a cron job monitoring service (image: healthchecks/healthchecks:latest). It listens on port 8000. Use an nginx reverse proxy. Use the shared PostgreSQL (host=zeltro-postgres, port=5432, user=root, password=password).

Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE healthchecks;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/healthchecks
3. Generate SECRET_KEY: $(openssl rand -hex 32)
4. Write docker-compose.yaml: healthchecks-app service (image: healthchecks/healthchecks:latest; env: DB=postgres, DB_HOST=zeltro-postgres, DB_PORT=5432, DB_NAME=healthchecks, DB_USER=root, DB_PASSWORD=password, SECRET_KEY=<generated>, SITE_ROOT=http://healthchecks, SITE_NAME=Healthchecks, EMAIL_HOST=zeltro-mailhog, EMAIL_PORT=1025, EMAIL_USE_TLS=False, DEFAULT_FROM_EMAIL=hc@example.com, SUPERUSER_EMAIL=admin@example.com, SUPERUSER_PASSWORD=admin123) + nginx (container_name=healthchecks, static VPC IP). Write nginx.conf proxying to healthchecks-app:8000.
5. cd ~/zeltro-projects/healthchecks && zeltro setup healthchecks --no-startup && zeltro up healthchecks
6. curl -sI http://healthchecks/ — expect 200 or 302.'"$SUMMARY_SUFFIX"

IDEA_NETBOX='Deploy NetBox on this Zeltro server. The project name is netbox — use this exact name, no changes.
...
Steps:
1. Create DB: docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE netbox;" 2>/dev/null || true
2. mkdir -p ~/zeltro-projects/netbox
3. Generate SECRET_KEY (50 chars): $(openssl rand -base64 37 | tr -d "=+/" | head -c 50); SUPERUSER_API_TOKEN=$(openssl rand -hex 20)
4. Write docker-compose.yaml: netbox-app service (image: netboxcommunity/netbox:latest; env: DB_HOST=zeltro-postgres, DB_PORT=5432, DB_NAME=netbox, DB_USER=root, DB_PASSWORD=password, REDIS_HOST=zeltro-redis, REDIS_PORT=6379, REDIS_PASSWORD=, REDIS_DB=4, REDIS_CACHE_DB=5, SECRET_KEY=<generated>, SUPERUSER_NAME=admin, SUPERUSER_EMAIL=admin@example.com, SUPERUSER_PASSWORD=admin123, SUPERUSER_API_TOKEN=<generated>, ALLOWED_HOSTS=netbox; volumes: netbox-media:/opt/netbox/netbox/media, netbox-reports:/opt/netbox/netbox/reports, netbox-scripts:/opt/netbox/netbox/scripts) + nginx (container_name=netbox, static VPC IP). Write nginx.conf proxying to netbox-app:8080.
5. cd ~/zeltro-projects/netbox && zeltro setup netbox --no-startup && zeltro up netbox
6. Wait 60 seconds for migrations to run, then curl -sI http://netbox/ — expect 200 or 302.'"$SUMMARY_SUFFIX"

IDEA_LABELSTUDIO='Deploy Label Studio on this Zeltro server. The project name is label-studio — use this exact name, no changes.
...
Steps:
1. mkdir -p ~/zeltro-projects/label-studio
...
3. cd ~/zeltro-projects/label-studio && zeltro setup label-studio --no-startup && zeltro up label-studio
