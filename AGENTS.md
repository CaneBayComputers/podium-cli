...
| `podium new <framework> <name>` | Greenfield project. Framework + name are required positionals (`laravel`, `kavera`, `wordpress`, `php`, `fastapi`, `flask`, `django`, `python`, `express`, `nestjs`, `fastify`, `node`). DB auto-selected per framework; override with `--database` (`mysql`, `postgres`, `mongodb`, `sqlite`); version via `--version`. |
