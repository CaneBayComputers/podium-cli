...
framework_scaffold() {
    echo-return; echo-cyan "Hono project selected!"
...
EOF
    ...
    if [[ "$JSON_OUTPUT" == "1" ]]; then
        ...
        git commit -m "Initial Hono project setup" > /dev/null 2>&1
    else
        git init; git add .; git commit -m "Initial Hono project setup"
    fi

    echo-green "Hono project structure created!"
}
...
GITEOF
    ...
    echo-green ".gitignore created for Hono project!"
}
