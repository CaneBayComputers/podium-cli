...
**Database**: PostgreSQL `codimd` on zeltro-postgres
...
- `CMD_HSTS_ENABLE=false` is set deliberately. HSTS is on by default and would pin browsers to https for this hostname, which Zeltro does not serve.
...
- The installer exists: run `zeltro install codimd`.
