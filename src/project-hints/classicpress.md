...
**Database**: MariaDB (`zeltro-mariadb`, database `classicpress`, user `root`, empty password)
...
- `CLASSICPRESS_DB_HOST` needs the port: `zeltro-mariadb:3306`. The database must already exist (the installer creates it); the image will not create it.
...
- The installer exists: run `zeltro install classicpress`.
