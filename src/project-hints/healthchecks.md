...
- `ALLOWED_HOSTS: healthchecks,localhost,127.0.0.1` — must include the Zeltro hostname.
- `SITE_ROOT: http://healthchecks` — used in ping URLs shown to users.
- `REGISTRATION_OPEN: "False"` — prevents public signups in local dev.
- Email: `EMAIL_HOST: zeltro-mailhog`, `EMAIL_PORT: 1025`, `EMAIL_USE_TLS: "False"`.
- Create DB first: `docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE healthchecks;"`
- Persist `healthchecks-data` volume.
- The installer exists: run `zeltro install healthchecks`.
