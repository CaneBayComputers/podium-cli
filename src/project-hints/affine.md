...
**Database**: PostgreSQL (`zeltro-postgres`, db `affine`) + Redis (`zeltro-redis`)
**Credentials**: Register on first visit — you land on `/admin/setup` to create the server owner

## Key Notes
- Upstream's compose ships `pgvector/pgvector:pg16`, but **pgvector is not required** with `AFFINE_INDEXER_ENABLED=false`. Verified: all 8 migrations and the server boot clean on Zeltro's vanilla PostgreSQL 17.
...
- The installer exists: run `zeltro install affine`.
