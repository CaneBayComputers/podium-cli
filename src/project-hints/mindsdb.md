...
- `MINDSDB_DB_CON` can repoint internal metadata at `zeltro-postgres`, but the default SQLite is used here because a bad value bricks startup with no useful error.
...
- The installer exists: run `zeltro install mindsdb`.
