...
- Campfire is SQLite-only by design — the database *and* uploaded attachments live in `/rails/storage`. It cannot use `zeltro-postgres`/`zeltro-mariadb`; that one volume is the whole backup.
...
- The installer exists: run `zeltro install once-campfire`.
