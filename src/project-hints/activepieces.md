...
**Database**: PostgreSQL (`zeltro-postgres`, db `activepieces`) + Redis (`zeltro-redis`)
**Credentials**: Register on first visit; the first account becomes the platform admin

## Key Notes
- Upstream's compose ships `pgvector/pgvector:0.8.0-pg14`, but **the community edition does not need pgvector**. Verified: it boots and runs clean against Zeltro's vanilla PostgreSQL 17 (`Activepieces Edition: ce`). pgvector is only wanted for the optional `AP_TOOL_SEARCH_ENABLED` semantic search, which defaults to off.
...
- The installer exists: run `zeltro install activepieces`.
