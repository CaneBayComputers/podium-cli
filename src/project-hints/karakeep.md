...
## Key Notes
- The two sidecars are not optional extras: `meilisearch` backs full-text search and `chrome` does the page crawling and screenshots. Neither is a Zeltro shared service, so both are bundled here. `MEILI_MASTER_KEY` must be identical in the app and the Meilisearch container.
- Upstream names its app service `web`, which Zeltro would treat as the entry point even though it listens on 3000. It is renamed `karakeep-app` here so the nginx proxy is picked up instead.
...
- The installer exists: run `zeltro install karakeep`.
