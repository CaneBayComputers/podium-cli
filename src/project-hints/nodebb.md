...
**Database**: PostgreSQL (`zeltro-postgres`, user=root, password=password, db `nodebb`)
**Credentials**: `admin` / `admin123456`

## Key Notes
- **Use PostgreSQL, not `zeltro-mongo`.** NodeBB builds its Mongo URI without `authSource`, so it authenticates against the `nodebb` database instead of `admin` and the shared root user always fails auth. Postgres needs no extensions — NodeBB creates its own `legacy_*` tables.
...
- The installer exists: run `zeltro install nodebb`.
