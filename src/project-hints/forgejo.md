...
- Create the DB before first start: `docker exec zeltro-mariadb mariadb -u root -e "CREATE DATABASE IF NOT EXISTS forgejo;"`
- Git-over-SSH is not exposed (Zeltro publishes HTTP only) — clone over HTTP.
- `client_max_body_size 512M` in the proxy, otherwise large pushes fail with 413.
- The installer exists: run `zeltro install forgejo`.
