...
**Database**: PostgreSQL (`zeltro-postgres`, user=root, password=password) + Redis (`zeltro-redis`)
...
- `REDASH_DATABASE_URL: postgresql://root:password@zeltro-postgres:5432/redash` — full connection URL format.
- `REDASH_REDIS_URL: redis://zeltro-redis:6379/3` — use a non-default Redis DB (e.g. /3) to avoid collision with other services.
...
- Create DB first: `docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE redash;"`
- No volumes needed (all state in PostgreSQL).
- The installer exists: run `zeltro install redash`.
