...
**Database**: PostgreSQL (`zeltro-postgres`, db `open_archiver`) + Redis (`zeltro-redis`) + a bundled Meilisearch
...
- **Do not set `REDIS_PASSWORD` or `REDIS_USER`.** Upstream's compose runs Valkey with `--requirepass`; `zeltro-redis` has no auth, and sending AUTH to an unauthenticated Redis makes BullMQ fail at boot.
- Meilisearch is bundled per-project (upstream pins `v1.38`) rather than using Zeltro's optional shared `zeltro-meilisearch`, which is behind a compose profile and off by default.
...
- The installer exists: run `zeltro install open-archiver`.
