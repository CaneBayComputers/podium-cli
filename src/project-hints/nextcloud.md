...
## Key Notes
- Single-service compose — Zeltro assigns the VPC IP directly to the nextcloud container.
- Env: `MYSQL_HOST=zeltro-mariadb`, `MYSQL_DATABASE=nextcloud`, `MYSQL_USER=root`, `MYSQL_PASSWORD=` (empty).
- Add Redis: `REDIS_HOST=zeltro-redis`, `REDIS_HOST_PORT=6379`.
...
- The installer exists: run `zeltro install nextcloud`.
