...
**Database**: MongoDB (`zeltro-mongo`, db `checkmate`)
...
- Zeltro's MongoDB requires auth, so the connection string needs credentials **and** `authSource=admin`: `mongodb://root:password@zeltro-mongo:27017/checkmate?authSource=admin`. Omitting `authSource` fails with an auth error.
...
- The installer exists: run `zeltro install checkmate`.
