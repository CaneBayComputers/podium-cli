...
**Database**: PostgreSQL (`zeltro-postgres`, db `pgbackweb`) for its own metadata
...
- `?sslmode=disable` is required: `zeltro-postgres` does not serve TLS and the Go driver otherwise tries to negotiate it.
...
- The image supports pg_dump for PostgreSQL 13–18, so it can back up Zeltro's own `zeltro-postgres` (17).
- The installer exists: run `zeltro install pgbackweb`.
