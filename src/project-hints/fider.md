...
**Database**: PostgreSQL (`zeltro-postgres`, user=root, password=password), database `fider`
**Credentials**: Create the site and admin account on the first-visit setup page

## Key Notes
- Fider **panics on boot** if `EMAIL_SMTP_HOST` and `EMAIL_SMTP_PORT` are unset — it only checks that they are present, never that they connect, so pointing at `zeltro-mailhog:1025` fully satisfies it.
...
- `ALLOW_PRIVATE_NETWORK_TARGETS=true` relaxes the SSRF guard so webhooks can reach other Zeltro containers.
- The installer exists: run `zeltro install fider`.
