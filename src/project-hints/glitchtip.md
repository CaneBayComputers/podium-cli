...
**Database**: PostgreSQL (`zeltro-postgres`, db `glitchtip`) + Redis (`zeltro-redis`)
...
- `EMAIL_URL` and `DEFAULT_FROM_EMAIL` are required; pointing `EMAIL_URL` at `smtp://zeltro-mailhog:1025` puts invites and alerts in Mailhog. `consolemail://` also works if you just want them logged.
...
- The installer exists: run `zeltro install glitchtip`.
