...
## Key Notes
- The backend service is deliberately named `notifier`, **not** `apprise-app` or similar: Zeltro's web-service regex is `^(nginx|web|app|api|server|frontend|backend|http)` and any name starting with "apprise" matches on `app`, which would hand the project IP to the wrong container.
...
- The installer exists: run `zeltro install apprise-api`.
