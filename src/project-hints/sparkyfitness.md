...
**Database**: PostgreSQL (`zeltro-postgres`, db `sparkyfitness`)
...
- The backend needs **two** DB identities: `SPARKY_FITNESS_DB_USER` (a superuser used for schema creation and migrations — Zeltro's `root` works) and `SPARKY_FITNESS_APP_DB_USER`/`_PASSWORD`, a limited role the backend creates itself on first boot. Both are mandatory.
...
- `SPARKY_FITNESS_FRONTEND_URL` drives CORS and Better Auth's trusted origins; it must equal `http://sparkyfitness`. `ALLOW_PRIVATE_NETWORK_CORS=true` is needed for private-LAN access like Zeltro's VPC.
...
- The installer exists: run `zeltro install sparkyfitness`.
