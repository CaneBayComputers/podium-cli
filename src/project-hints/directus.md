...
- Redis for caching: `CACHE_ENABLED: "true"`, `CACHE_STORE: redis`, `REDIS: redis://zeltro-redis:6379`.
- `WEBSOCKETS_ENABLED: "true"` — nginx must include WebSocket upgrade headers.
- `PUBLIC_URL: http://directus` must match the Zeltro hostname exactly.
- Create DB first: `docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE directus;"`
...
- The installer exists: run `zeltro install directus`.
