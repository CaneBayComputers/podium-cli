...
**Database**: PostgreSQL (`zeltro-postgres`, user=root, password=password, db `owncloud`)
**Credentials**: `admin` / `admin123`

## Key Notes
- **Use `pgsql`, not MariaDB.** The `/usr/bin/owncloud` entrypoint only passes `--database-pass` when the password is non-empty, so `zeltro-mariadb`'s empty root password makes `occ maintenance:install` fall into an interactive prompt under `--no-interaction`. Postgres has a real password and sidesteps it.
...
- The installer exists: run `zeltro install owncloud`.
