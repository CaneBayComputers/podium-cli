...
## Key Notes
- **CPU-only.** No `deploy.resources.devices` GPU reservation is declared, because Zeltro injects the network and a GPU request would fail on hosts without the NVIDIA runtime. Add one manually if you have a GPU.
...
- Service names are `ollama`, `openwebui-app` and `nginx`. The front-end is deliberately **not** called `open-webui` so it cannot collide with Zeltro's entry-point name detection.
...
- The installer exists: run `zeltro install ollama-with-open-webui`.
