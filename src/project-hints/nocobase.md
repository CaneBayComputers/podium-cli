...
**Database**: PostgreSQL, `nocobase` on zeltro-postgres
...
- NocoBase's own compose runs Postgres with `wal_level=logical`; the shared zeltro-postgres does not. Only the change-data-capture plugins need it, so the base install is unaffected.
- The installer exists: run `zeltro install nocobase`.
