...
- `MAIL_DRIVER: smtp` with `MAIL_HOST: zeltro-mailhog` and `MAIL_PORT: 1025` for local email.
- Create DB first: `docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE cachet;"`
- No volumes needed (all state lives in the database).
- The installer exists: run `zeltro install cachet`.
