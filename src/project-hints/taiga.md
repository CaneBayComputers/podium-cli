...
**Database**: PostgreSQL (`zeltro-postgres`) + 2× RabbitMQ sidecars + Redis (`zeltro-redis`)
...
- The gateway nginx (not a separate Zeltro nginx) handles all routing — assign the VPC IP to `taiga-gateway`.
- `taiga-gateway/taiga.conf` routes `/api/` and `/admin/` to back, `/events` to events, `/media/` to protected, `/static/` from volume.
- `POSTGRES_HOST=zeltro-postgres` (not a local service).
- `EMAIL_BACKEND=console` for dev (no actual email sending).
- The installer exists: run `zeltro install taiga`.
