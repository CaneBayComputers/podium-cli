...
- MariaDB must exist before first start: `docker exec zeltro-mariadb mariadb -u root -e "CREATE DATABASE IF NOT EXISTS gitea;"`
- Set `GITEA__server__ROOT_URL=http://gitea/` and `GITEA__server__HTTP_PORT=3000`.
- Persist `/data` volume for repos, keys, and avatars.
- The installer exists: run `zeltro install gitea`.
