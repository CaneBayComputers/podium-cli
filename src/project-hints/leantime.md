...
- `LEAN_DB_PASSWORD: ""` — the Zeltro MariaDB root user has no password.
- `LEAN_APP_URL: http://leantime` must match the Zeltro hostname.
- `LEAN_SESSION_PASSWORD` can be any random string (used to sign sessions).
- Create DB before start: `docker exec zeltro-mariadb mariadb -u root -e "CREATE DATABASE IF NOT EXISTS leantime;"`
- Persist `leantime-userfiles`, `leantime-public-userfiles`, `leantime-plugins`, `leantime-logs` volumes.
- The installer exists: run `zeltro install leantime`.
