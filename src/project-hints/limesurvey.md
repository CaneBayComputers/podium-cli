...
```bash
docker exec zeltro-mariadb mysql -u root -e "CREATE DATABASE IF NOT EXISTS limesurvey; CREATE USER IF NOT EXISTS 'limesurvey'@'%' IDENTIFIED BY 'limesurvey'; GRANT ALL PRIVILEGES ON limesurvey.* TO 'limesurvey'@'%'; FLUSH PRIVILEGES;"
```
...
3. `cd ~/zeltro-projects/limesurvey && zeltro setup limesurvey --no-startup`
4. `zeltro up limesurvey`
