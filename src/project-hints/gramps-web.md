...
**Database**: Gramps' own SQLite/BSDDB tree in volumes + Redis (`zeltro-redis`, dbs 10/11) for Celery and rate limiting
...
- Upstream bundles Valkey; here Celery points at the shared Redis instead: `GRAMPSWEB_CELERY_CONFIG__broker_url=redis://zeltro-redis:6379/10`, `..._result_backend` the same, `GRAMPSWEB_RATELIMIT_STORAGE_URI=redis://zeltro-redis:6379/11`.
...
- The installer exists: run `zeltro install gramps-web`.
