...
**Database**: ClickHouse sidecar for events + shared `zeltro-redis`. No MySQL/Postgres is used.
...
- ClickHouse must be bundled: it is the event store and is not one of Zeltro's shared services. The four XML overlays cut its RAM and disable query logging; without them it is very memory hungry.
- `CLICKHOUSE_PASSWORD` must be identical on the ClickHouse container and the API, and the API refuses to start without it.
- Redis points at the shared `zeltro-redis`; only ClickHouse is local to the project.
...
- The installer exists: run `zeltro install swetrix`.
