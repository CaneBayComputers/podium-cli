...
## Key Notes
- Glance **exits on a malformed config**. The installer ships a known-good minimal `glance.yml`; validate edits with `zeltro logs glance` after saving.
...
- The installer exists: run `zeltro install glance`.
