...
**Database**: PostgreSQL (`zeltro-postgres`, user=root, password=password, db `onedev`)
...
- Git-over-SSH lives on 6611 and is not published by Zeltro — clone over HTTP. Set `ssh_port=0` to disable the listener.
- Persist `/opt/onedev` (repos, build artifacts, config). Migrations run in-process on every start.
- The installer exists: run `zeltro install onedev`.
