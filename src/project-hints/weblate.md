...
**Database**: PostgreSQL (`zeltro-postgres`, db `weblate`, user=root, password=password) + `zeltro-redis` (db 1)
**Credentials**: admin / admin123

## Key Notes
- The app service is named **`translate-server`**, not `weblate-app`. Zeltro's web-service regex is `^(nginx|web|app|api|server|frontend|backend|http)` and *any* `weblate-*` name matches on `web`, which would give the project IP to the app container instead of nginx. Don't rename it back.
...
- The installer exists: run `zeltro install weblate`.
