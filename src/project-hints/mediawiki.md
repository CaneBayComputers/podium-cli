...
**Database**: MariaDB `mediawiki` on zeltro-mariadb
...
- That init service is guarded by `test -f`, so later `zeltro up` runs skip it. To re-run setup from scratch, delete the `mediawiki-config` volume and the `mediawiki` database.
...
- To add extensions or tweak settings, edit the file inside the volume:
  `zeltro exec mediawiki vi /conf/LocalSettings.php`
- The installer exists: run `zeltro install mediawiki`.
