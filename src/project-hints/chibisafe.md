...
- The frontend service is named `chibisafe-frontend`, not `chibisafe`. A service named exactly `chibisafe` would collide with the `container_name: chibisafe` Zeltro puts on the entry point, giving one hostname two containers and randomly routed requests.
...
- The installer exists: run `zeltro install chibisafe`.
