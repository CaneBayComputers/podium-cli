...
**Port**: 80
**Database**: MariaDB (`zeltro-mariadb`, db `pterodactyl`) + Redis (`zeltro-redis`) for cache/session/queue
...
- Actually running game servers needs **Wings** on a host with Docker socket access, its own ports, and a real node registration — out of scope for a Zeltro project. The panel alone is fully usable for browsing/administration.
- The installer exists: run `zeltro install pterodactyl`.
