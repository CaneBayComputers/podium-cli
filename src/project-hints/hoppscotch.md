...
**Database**: PostgreSQL (`zeltro-postgres`, db `hoppscotch`, user=root, password=password)
**Credentials**: the first account that signs in at `/admin` becomes the administrator

## Key Notes
- `ENABLE_SUBPATH_BASED_ACCESS: "true"` is what makes this fit Zeltro at all. In the default "multiport" mode the AIO image binds 3000, 3100 and 3170 separately and needs three exposed ports; subpath mode collapses them onto port 80 as `/`, `/admin` and `/backend`.
...
- Sign-in needs an auth provider. Email magic-link needs SMTP, configurable from the admin dashboard — point it at `zeltro-mailhog:1025` and read the mail there.
- The installer exists: run `zeltro install hoppscotch`.
