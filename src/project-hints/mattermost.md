...
## Key Notes
- Env: `MM_SQLSETTINGS_DRIVERNAME=postgres`, `MM_SQLSETTINGS_DATASOURCE=postgres://root:password@zeltro-postgres:5432/mattermost?sslmode=disable`.
- Set `MM_SERVICESETTINGS_SITEURL=http://mattermost` and `MM_SERVICESETTINGS_LISTENADDRESS=:8065`.
- Create DB first: `docker exec zeltro-postgres psql -U root -d postgres -c "CREATE DATABASE mattermost;"`
...
- The installer exists: run `zeltro install mattermost`.
