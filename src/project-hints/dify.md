...
**Database**: PostgreSQL (`zeltro-postgres`) + Redis (`zeltro-redis`) + Weaviate sidecar (vector store)
...
- Uses `.env` file for configuration; key vars: `SECRET_KEY`, `DB_HOST=zeltro-postgres`, `REDIS_HOST=zeltro-redis`.
...
- `CELERY_BROKER_URL=redis://zeltro-redis:6379/1` (uses Redis DB 1 for Celery).
- Storage is local filesystem; uploaded files stored in `dify-storage` volume.
- The installer exists: run `zeltro install dify`.
