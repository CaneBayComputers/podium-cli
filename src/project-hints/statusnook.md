...
- Upstream's compose binds the published port to `127.0.0.1` — irrelevant under Zeltro, which routes over the VPC network instead.
- Persist `/app/statusnook-data`; it holds the SQLite database and the config.
- The installer exists: run `zeltro install statusnook`.
