...
**Database**: MongoDB (`zeltro-mongo`, root/password) + shared `zeltro-redis`
...
- MongoDB auth needs `?authSource=admin` since the shared `zeltro-mongo` root user lives in the admin database.
...
- The installer exists: run `zeltro install heyform`.
