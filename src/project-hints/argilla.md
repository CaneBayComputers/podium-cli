...
**Database**: PostgreSQL (`zeltro-postgres`, database `argilla`) + Redis (`zeltro-redis`, db 12) + bundled Elasticsearch
...
- Elasticsearch is bundled because Zeltro has no shared search engine. `discovery.type=single-node` skips the bootstrap checks, so **no host `vm.max_map_count` change is needed**; `node.store.allow_mmap=false` avoids the mmap ceiling that check would have caught.
- `ARGILLA_DATABASE_URL` uses the async driver: `postgresql+asyncpg://root:password@zeltro-postgres:5432/argilla`.
...
- The installer exists: run `zeltro install argilla`.
