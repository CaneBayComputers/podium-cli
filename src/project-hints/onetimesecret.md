...
**Database**: `zeltro-redis` (db 0) — secrets live only in Redis
...
- Secrets are stored in shared `zeltro-redis` db 0 with no persistence guarantees — a Redis flush wipes them.
...
- The installer exists: run `zeltro install onetimesecret`.
