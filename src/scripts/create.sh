...
23. When a project ships its own multi-service docker-compose (e.g. it defines its own database, cache, worker, or uses a specialized Docker image rather than a plain PHP/Node/Python runtime), do NOT try to squeeze it into a Podium cbc container. Instead, adapt the project's own docker-compose to work with Podium's infrastructure using this approach:
   a. Run `podium clone <url> --no-github --overwrite-docker-compose` to let Podium register the project: this assigns a unique IP address, port, and /etc/hosts entry. Read the generated docker-compose.yaml to note the assigned IP_ADDRESS, PROJECT_PORT (container_name), and CONTAINER_NAME.
   b. Retrieve the project's original docker-compose from git: `git show HEAD:docker-compose.yml` (or `docker-compose.yaml` — check which filename the project uses).
   c. Build a replacement docker-compose.yaml based on the original with these changes:
      - On the main web-facing service: add the IP and port Podium assigned (from step a), and wire it to the podium-cli_vpc network with that static IP.
      - For any service the project defines that Podium already provides (postgres, mysql, mariadb, redis, mongodb, memcached): remove that service block entirely and update the app's environment variables to point at the Podium shared container instead (hostnames: podium-mariadb, podium-postgres, podium-redis, podium-mongo).
      - Keep all other project-specific services (workers, schedulers, search engines, etc.) as-is, connected to podium-cli_vpc (without a fixed IP — only the web-facing service needs one).
      - Add the podium-cli_vpc network as external at the bottom: `networks: default: external: true  name: podium-cli_vpc`
   d. Write this adapted compose as docker-compose.yaml, replacing the Podium-generated one.
   e. Update the project's environment configuration (its .env or config files) to use the Podium shared service hostnames.
   f. Start the project with `podium up <project-name>` and verify with curl.
