...
26. If no installer exists, check if `src/project-hints/<project-slug>.md` exists in the Podium CLI directory (the directory you read README.md from). The project slug is the lowercase hyphenated name (e.g., `strapi`, `netbox`, `ghost`). If the file exists, read it first and follow any project-specific notes there.
